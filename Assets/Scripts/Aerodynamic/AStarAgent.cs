...
public class AStarAgent : MonoBehaviour
{
    ...
    Point _start;
    public Point _end {get; private set;}
    ...
    private List<Point> ReconstructPath(PointData start, PointData current, PointData[][][] dataSet)
    {
        ...
        Point currentPoint = PathfindingWorldManager.Instance.Grid[current.Coords.x][current.Coords.y][current.Coords.z];

        currentPoint.AddMovingData(this, currentPointData.TimeToReach);
        totalPath.Add(currentPoint);

        Point cameFromPoint = PathfindingWorldManager.Instance.Grid[current.CameFrom.x][current.CameFrom.y][current.CameFrom.z];
        ...
        while (current.CameFrom.x != -1 && count < 10000)
        {
            ...
            currentPoint = PathfindingWorldManager.Instance.Grid[current.Coords.x][current.Coords.y][current.Coords.z];
            PointData cameFromPointData = dataSet[current.CameFrom.x][current.CameFrom.y][current.CameFrom.z];
            cameFromPoint = PathfindingWorldManager.Instance.Grid[current.CameFrom.x][current.CameFrom.y][current.CameFrom.z];
    ...
    public AStarAgentStatus Pathfinding(Vector3 goal,bool supressMovement=false)
    {
        ...
        _endPosition = goal;
        _start = PathfindingWorldManager.Instance.GetClosestPointWorldSpace(transform.position);
        _end = PathfindingWorldManager.Instance.GetClosestPointWorldSpace(goal);
        ...
        PointData[][][] dataSet = new PointData[PathfindingWorldManager.Instance.Grid.Length][][];
        for (int i = 0; i < dataSet.Length; i++)
        {
            dataSet[i] = new PointData[PathfindingWorldManager.Instance.Grid[i].Length][];
            for (int j = 0; j < dataSet[i].Length; j++)
            {
                dataSet[i][j] = new PointData[PathfindingWorldManager.Instance.Grid[i][j].Length];
            }
        }
        ...
        while (openSet.Count > 0)
        {
            ...
            for (int i = 0; i < currentPoint.Neighbours.Count; i++)
            {
                ...
                Point neighbour = PathfindingWorldManager.Instance.Grid[indexes.x][indexes.y][indexes.z];
                ...
                {
                    float tenativeScore = current.GScore + PathfindingWorldManager.Instance.PointDistance;
    ...
    IEnumerator Coroutine_RePath()
    {
        ...
        Point p = PathfindingWorldManager.Instance.GetClosestPointWorldSpace(transform.position);
    ...
    IEnumerator Coroutine_CharacterFollowPath()
    {
        ...
        for (int i = TotalPath.Count - 1; i >= 0; i--)
        {
            ...
            while (l<length)
            {
                ...
                if (CurvePath)
                {
                    transform.position += transform.forward * controller.getScaledTimeDelta() * Speed;
                    transform.forward = Vector3.Slerp(transform.forward, forwardDirection, controller.getScaledTimeDelta() * TurnSpeed * Mathf.Deg2Rad);
                }
                else
                {
                    transform.forward = forwardDirection;
                    transform.position = Vector3.MoveTowards(transform.position, TotalPath[i].WorldPosition, controller.getScaledTimeDelta() * Speed);
                }
                l += controller.getScaledTimeDelta() * Speed;
